...
export default function ProfesorDashboard({ onLogout, testActivo }) {
  ...
  // Si hay un test activo en Firebase y no tenemos código local, usarlo automáticamente
  useEffect(() => {
    if (testActivo?.codigo && !codigoActual) {
      setCodigoActual(testActivo.codigo)
    }
  }, [testActivo])
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
          ...
          {tab === 'participar' && (
            <>
              ...
              <Metricas respuestas={respuestas} preguntas={preguntas.length > 0 ? preguntas : (testActivo?.preguntas || [])} />
            </>
          )}
